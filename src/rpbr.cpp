...
#define         PATH_ICON                   "../release/resources/textures/rpbr_icon.png"                      // Path to rPBR icon to replace default window icon
#define         PATH_MODEL                  "../release/resources/models/cerberus.obj"                         // Path to default OBJ model to load
#define         PATH_TEXTURES_HDR           "../release/resources/textures/hdr/pinetree.hdr"                   // Path to default HDR environment map to load
#define         PATH_TEXTURES_ALBEDO        "../release/resources/textures/cerberus/cerberus_albedo.png"       // Path to default PBR albedo texture
#define         PATH_TEXTURES_NORMALS       "../release/resources/textures/cerberus/cerberus_normals.png"      // Path to default PBR tangent normals texture
#define         PATH_TEXTURES_METALNESS     "../release/resources/textures/cerberus/cerberus_metalness.png"    // Path to default PBR metalness texture
#define         PATH_TEXTURES_ROUGHNESS     "../release/resources/textures/cerberus/cerberus_roughness.png"    // Path to default PBR roughness texture
// #define      PATH_TEXTURES_AO            "../release/resources/textures/cerberus/cerberus_ao.png"           // Path to default PBR ambient occlusion texture (NO AVAILABLE)
// #define      PATH_TEXTURES_EMISSION      "../release/resources/textures/cerberus/cerberus_emission.png"     // Path to default PBR emission texture  (NO AVAILABLE)
// #define      PATH_TEXTURES_HEIGHT        "../release/resources/textures/cerberus/cerberus_height.png"       // Path to default PBR parallax height texture  (NO AVAILABLE)
#define         PATH_SHADERS_POSTFX_VS      "../release/resources/shaders/postfx.vs"                           // Path to screen post-processing effects vertex shader
#define         PATH_SHADERS_POSTFX_FS      "../release/resources/shaders/postfx.fs"                           // Path to screen post-processing effects fragment shader
#define         PATH_GUI_STYLE              "../release/resources/rpbr_gui.style"                              // Path to GUI style data file
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        overUI = CheckCollisionPointRec(GetMousePosition(), (Rectangle){ static_cast<float>(GetScreenWidth() - UI_MENU_WIDTH), 0, UI_MENU_WIDTH, static_cast<float>(GetScreenHeight()) });
            ...
            else
            {
                ...
                if (supportedImage)
                {
                    for (int i = 0; i < MAX_TEXTURES; i++)
                    {
                        ...
                        Vector2 pos = { static_cast<float>(GetScreenWidth() - UI_MENU_WIDTH + UI_MENU_WIDTH/2), scrolling + UI_MENU_PADDING*2 + UI_MENU_PADDING*2.5f + 
                        ...
                        if (CheckCollisionPointRec(GetMousePosition(), rect))
                        {
                            ...
                            if (textures[i].id != 0) UnsetMaterialTexturePBR(&matPBR, static_cast<TypePBR>(i));
                            SetMaterialTexturePBR(&matPBR, static_cast<TypePBR>(i), newTex);
                            textures[i] = newTex;
                            break;
                        }
                    }
        ...
        if ((GetKeyPressed() == KEY_NUMPAD_SUM) && (renderScale < (MAX_RENDER_SCALES - 1))) {
            renderScale = static_cast<RenderScale>(renderScale+1);
        }
        else if ((GetKeyPressed() == KEY_NUMPAD_SUBTRACT) && (renderScale > 0)) {
            renderScale = static_cast<RenderScale>(renderScale-1);
        }
        ...
        for (int i = 0; i < MAX_TEXTURES; i++)
        {
            ...
            {
                UnsetMaterialTexturePBR(&matPBR, static_cast<TypePBR>(i));
        ...
        if (!canMoveCamera && ((!IsMouseButtonDown(MOUSE_MIDDLE_BUTTON) && (cameraType == static_cast<CameraType>(CAMERA_ORBITAL))))) canMoveCamera = true;
        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            ...
            for (int i = 0; i < totalLights; i++)
            {
                ...
                // if (CheckCollisionRaySphere(ray, lights[i].position, LIGHT_RADIUS)) selectedLight = i; // OLD
                RayCollision ray_col = GetRayCollisionSphere(ray, lights[i].position, LIGHT_RADIUS);
                if (ray_col.hit) selectedLight = i;
        ...
        BeginDrawing();
            ...
            BeginTextureMode(fxTarget);
                ...
                BeginMode3D(camera);
                    ...
                    if (drawLights) for (unsigned int i = 0; (i < totalLights); i++)
                    {
                        Ray ray = GetMouseRay(GetMousePosition(), camera);
                        RayCollision ray_col = GetRayCollisionSphere(ray, lights[i].position, LIGHT_RADIUS);
                        DrawLight(lights[i], ray_col.hit);
                        // DrawLight(lights[i], CheckCollisionRaySphere(ray, lights[i].position, LIGHT_RADIUS)); // OLD
                    }
            ...
            BeginShaderMode(fxShader);
                ...
                DrawTexturePro(fxTarget.texture, (Rectangle){ 0, 0, static_cast<float>(fxTarget.texture.width), static_cast<float>(-fxTarget.texture.height) }, 
                               (Rectangle){ 0, 0, static_cast<float>(GetScreenWidth()), static_cast<float>(GetScreenHeight()) }, (Vector2){ 0.0f, 0.0f }, 0.0f, WHITE);
